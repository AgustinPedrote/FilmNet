<x-app-layout>
    <h1 class="text-2xl font-bold mb-6 mt-20 ml-10 border-b-2 border-blue-500 w-11/12 pb-2 text-gray-800">
        Votaciones de {{ $usuario->name }} <span class="text-blue-500">({{ $votaciones->count() }})</span>
    </h1>

    <div class="max-w-7xl mx-auto sm:px-2 md:px-6 lg:px-8">
        @if ($votaciones->isEmpty())
            <p class="text-gray-500 text-lg text-center mt-8 mb-72">No hay votaciones disponibles.</p>
        @else
            <!-- Mostrar la media de las votaciones -->
            <div class="text-right text-green-600">
                <span class="text-lg font-semibold">Media Votaciones:</span>
                <span class="text-xl">{{ number_format($mediaVotos, 1) }}</span>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($votaciones->reverse() as $votacion)
                            <tr>
                                ...
                                <td class="hidden md:table-cell px-4 py-4 sm:py-6 text-base sm:text-lg">
                                    Tipo: {{ $votacion->audiovisual->tipo->nombre }}<br>
