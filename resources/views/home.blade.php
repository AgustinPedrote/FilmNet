    ...
    @foreach (['Películas nuevas' => $peliculas, 'Series nuevas' => $series, 'Documentales nuevos' => $documentales] as $title => $items)
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 justify-center mx-auto">
            ...
            @foreach ($items as $item)
                ...
                <a href="{{ route('audiovisual.show', $item) }}" class="group p-4 transition duration-300 ease-in-out transform hover:scale-105">
                    ...
                    <div class="relative w-full h-64 overflow-hidden rounded-md shadow-md">
                        ...
                        <img src="{{ $item->img }}" alt="{{ $item->titulo }}" class="object-cover w-full h-full transition duration-300 ease-in-out transform scale-100 group-hover:scale-110" />
                    </div>
